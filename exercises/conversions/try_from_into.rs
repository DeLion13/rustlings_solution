...
impl TryFrom<&str> for Person {
    ...
    fn try_from(s: &str) -> Result<Self, Self::Error> {
        if s.len() == 0 {
            Err(format!("Empty string"))
        } else {
            let new_vec: Vec<_> = s.split(",").collect::<Vec<&str>>();
            let age: usize = new_vec[1].parse::<usize>().unwrap_or(0);
            if age != 0 {
                Ok(Person {
                    name: String::from(new_vec[0]),
                    age: age,
                })
            } else {
                Err(format!("Format trouble"))
            }
        }
    }
}
